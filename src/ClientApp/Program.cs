...
builder.Services
    .AddSingleton<SomeGrpcInterceptor>()
    .AddSingleton<SomeDelegatingHandler>();
...
public class SomeBackgroundService(IServiceScopeFactory scopeFactory, ILogger<SomeBackgroundService> logger)
    : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        // out of the box, will hit the server app, including the traceparent header with sampled flag set to false
        // so the server app will not send the trace to the collector (unless we force always sampling on the server app)
        // I assume the traceparent header is sent in this case and not the above, because somewhere along the way
